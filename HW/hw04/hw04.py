...
def deep_map(f, s):
    ...
    "*** YOUR CODE HERE ***"
    # loop over s
    # for each index, if is number, modify in place, else apply  deepmap to it?
...
def balanced(m):
    ...
    # calculate torque of arm
    def torque(s):
        return length(s) * total_mass(end(s))

    def submobiles_balanced(s):
        if is_mobile(end(s)):
            return balanced(end(s))
        else:
            return True

    return (
        torque(left(m)) == torque(right(m))
        and submobiles_balanced(left(m))
        and submobiles_balanced(right(m))
    )
...
def max_path_sum(t):
    ...
    # otherwise return label + recursive call
    highest_sum = 0
    if is_leaf(t):
        return label(t)
    else:
        for b in branches(t):
            highest_sum = max(highest_sum, max_path_sum(b))
